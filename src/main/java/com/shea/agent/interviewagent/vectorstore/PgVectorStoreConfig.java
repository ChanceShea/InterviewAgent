...
public class PgVectorStoreConfig {
    ...
    protected void addDocumentsWithTransaction1(
            VectorStore vectorStore,
            List<Document> fileChunks) {

        for (int i = 0; i < fileChunks.size(); i++) {
            Document document = fileChunks.get(i);
            String text = document.getText();

            try {
                log.info(
                        "准备添加 chunk，index={}，id={}，chars={}，semanticChars={}，text={}",
                        i,
                        document.getId(),
                        text == null ? -1 : text.length(),
                        countSemanticChars(text),
                        abbreviate(text, 300)
                );
                ...
                log.error(
                        "chunk 添加失败，index={}，id={}，chars={}，text={}",
                        i,
                        document.getId(),
                        text == null ? -1 : text.length(),
    ...
    private long countSemanticChars(String text) {
        if (text == null) {
            return 0;
        }
